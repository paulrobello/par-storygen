...
**Optional web surface:** the package also ships a FastAPI server (`src/storygen_api/`, installed via the `[api]` extra; console script `storygen-api`) and a Next.js frontend (`web/`). The API is a second composition root over the same `storage`/`llm`/`images`/`pipeline` layers — see the "Web surface" section of `docs/ARCHITECTURE.md`. `web/` is excluded from pyright (`tool.pyright.exclude = ["web"]`). The API binds `127.0.0.1` by default (loopback only) and gates every state-changing / cost-incurring route behind a shared bearer token when `STORYGEN_API_TOKEN` is set (fail-closed `503` when the variable is unset on a protected route); run single-worker (`--workers 1`) — in-memory session/WebSocket/TTS state does not survive a second worker. See `AUDIT.md` (SEC-001/SEC-006) before exposing it beyond loopback.
