...
2. Use default save directory `~/Desktop/<sanitized_title> - Book/` (auto-created if missing; appends `(N)` suffix if already exists)
...
5. Copy TTS audio files (`audio/<id>-<provider>-<voice_hash>.<ext>`) → `audio/<id>-<provider>-<voice_hash>.<ext>` for nodes that have them
6. Collect character portraits referenced by `featured_character_ids` on each node
7. Render Jinja2 template with collected data, write `index.html`
8. Open in default browser via `webbrowser.open()`
9. Show toast notification with export path

## Output Structure

```
~/Desktop/<Story Title> - Book/
├── index.html
├── images/
│   ├── node-abc123.png        # Scene illustrations
│   ├── portrait-char1-v1.png  # Character portraits
│   └── ...
└── audio/
...
- When active:
  ...
  - If current page has no audio: wait `auto_read_delay` seconds (default 8s), then advance
...
# Template context:
{
    ...
    "pages": list[BookPage],
    "has_any_audio": bool,
    "auto_read_delay": int,    # default 8
}
```

### Key Implementation Details

- `export_book()` is async-safe but does file I/O synchronously (acceptable for export)
- Run in a `@work(exit_on_error=False)` on PlayScreen
...
- Sanitize story title for directory name (replace special chars with dashes)
- Jinja2 template loaded via `importlib.resources` or `Path(__file__).parent / "template.html"`
