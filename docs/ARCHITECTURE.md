...
- `storage/app_state.py` — small JSON at `$XDG_CONFIG_HOME/storygen/state.json` keyed by:
  ...
  - `wizard_defaults` — a `WizardDefaults` dataclass (theme/tone_preset/tone_descriptor/narration_style/art_style/reader_level/characters/target_major_beats/save_to_catalog). The Settings screen edits these; the wizard reads them at launch and pre-fills every widget. Constants: `DEFAULT_ART_STYLE = "children's story book"`, `DEFAULT_TARGET_MAJOR_BEATS = 10`, `MIN/MAX_TARGET_MAJOR_BEATS = 2/30`, `DEFAULT_TONE_PRESET = "serious"`, `DEFAULT_NARRATION_STYLE = "third_person"`, `DEFAULT_READER_LEVEL = "ages_11_15"`.
